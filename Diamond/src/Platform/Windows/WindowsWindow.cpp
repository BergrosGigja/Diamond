...
namespace Diamond 
{
	...
	{
		DI_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		DI_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		// Only need to initialize glfw once, may have multiple windows though...
		if (!s_GLFWInitialized)
		{
			// TODO: glfwTerminate on system shutdown
			int success = glfwInit();
			DI_CORE_ASSERT(success, "Could not initialize GLFW!");
